...
class Core_App extends Zend_Application
{
     ...
     * @param array|null|string|Zend_Config $appPath
    ...
    )
    {
        ...
        $map               = new Zend_Loader_ClassMapAutoloader();

        $map->registerAutoloadMaps(
            [
        ...
        $map->register();
        $this->_autoloader->pushAutoloader($map);
        $standard = new Zend_Loader_StandardAutoloader();
        $standard->registerPrefixes(
            [
                'Core' => APPLICATION_PATH . '/../library/Core',
                'Zend' => APPLICATION_PATH . '/../library/Zend',
            ]
        );
        $standard->setFallbackAutoloader(true);
        $standard->register();
        $this->_autoloader->pushAutoloader($standard);
    ...
    private function _addRoutesFromConfig($options)
    {
        if (!isset($options['resources']['router']['routes'])) {
            $options['resources']['router']['routes'] = [];
        }
        ...
        if (isset($options['resources']['router']['useRequestParametersAsGlobal'])) {
            $this->_router->useRequestParametersAsGlobal(
                $options['resources']['router']['useRequestParametersAsGlobal']
            );
        }
