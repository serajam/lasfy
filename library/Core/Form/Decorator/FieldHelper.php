 ...
 * @subpackage Decorator
 */
class Core_Form_Decorator_FieldHelper extends Zend_Form_Decorator_Abstract
{
    /**
    ...
    public function render($content)
    {
        ...
        $tooltip = $element->getAttrib('fieldhelper');
        $tooltip = trim($tooltip);

        if (!empty($tooltip) && (null !== ($translator = $element->getTranslator()))) {
            $description = $translator->translate($tooltip);
        }
        ...
        if ($escape) {
            $description = $view->escape($tooltip);
        }
        $icon = '<i class="gen-enclosed foundicon-idea"></i>';
        ...
        if (!empty($tag)) {
            ...
            $description      = $decorator->render($icon);
        }
