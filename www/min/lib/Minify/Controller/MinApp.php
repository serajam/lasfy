            ...
            foreach ($keys as $key) {
                ...
                foreach ($files as $file) {
                    ...
                    if ($realpath && is_file($realpath)) {
                        ...
                        $this->log(
                            "The path \"{$file}\" (realpath \"{$realpath}\") could not be found (or was not a file)"
                        );
                        if (null === $firstMissingResource) {
                            ...
                            $this->log(
                                "More than one file was missing: '$firstMissingResource', '$secondMissingResource'"
                            );
        ...
        if (!$cOptions['groupsOnly'] && isset($_GET['f'])) {
            ...
            foreach ($files as $file) {
                ...
                if (false === $realpath || !is_file($realpath)) {
                    $this->log(
                        "The path \"{$path}\" (realpath \"{$realpath}\") could not be found (or was not a file)"
                    );
                    if (null === $firstMissingResource) {
            ...
            if (null !== $firstMissingResource) {
                array_unshift(
                    ...
                    new Minify_Source(array(
                        ...
                        ,
                        'content'      =>
                            "/* Minify: at least one missing file. See " . Minify::URL_DEBUG
                            . " */\n"
                        ,
                        'minifier'     => ''
                    ))
