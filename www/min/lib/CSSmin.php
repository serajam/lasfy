...
class CSSmin
{
    ...
    private function minify($css, $linebreak_pos)
    {
        ...
        $css = preg_replace_callback(
            ...
            array($this, 'preserve_old_IE_specific_matrix_definition'),
            $css
        );
        ...
        $css = preg_replace_callback(
            '/(@[a-z\-]*?keyframes[^\{]*?\{)(.*?\}\s*\})/iS',
            array($this, 'replace_keyframe_zero'),
            $css
        );
    ...
    private function compress_hex_colors($css)
    {
        ...
        while (preg_match($pattern, $css, $m, 0, $offset)) {
            ...
            if ($is_filter) {
                ...
                if (strtolower($m[2]) == strtolower($m[3]) && strtolower($m[4]) == strtolower($m[5])
    ...
    private function preserve_old_IE_specific_matrix_definition($matches)
    {
        ...
        return 'filter:progid:DXImageTransform.Microsoft.Matrix(' . self::TOKEN . (count($this->preserved_tokens) - 1)
        . '___' . ')';
    }
