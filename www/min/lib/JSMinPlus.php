...
class JSMinPlus
{
    ...
    public function parseTree($n, $noBlockGrouping = false)
    {
        ...
        switch ($n->type) {
            ...
            case KEYWORD_SWITCH:
                ...
                for ($i = 0, $j = count($cases); $i < $j; $i++) {
                    ...
                    if ($case->type == KEYWORD_CASE) {
                        $s .= 'case' . ($case->caseLabel->type != TOKEN_STRING ? ' ' : '') . $this->parseTree(
                                $case->caseLabel
                            ) . ':';
                    } else {
                        $s .= 'default:';
            ...
            case JS_FOR_IN:
                $s = 'for(' . ($n->varDecl ? $this->parseTree($n->varDecl) : $this->parseTree($n->iterator)) . ' in '
            ...
            case JS_INDEX:
                ...
                if ($n->treeNodes[0]->type == TOKEN_IDENTIFIER && $n->treeNodes[1]->type == TOKEN_STRING
                    ...
                    if ($t->type == JS_PROPERTY_INIT) {
                        ...
                        if ($t->treeNodes[0]->type == TOKEN_STRING
                            && $this->isValidIdentifier(
                                substr($t->treeNodes[0]->value, 1, -1)
                            )
                        ) {
...
class JSParser
{
        ...
        switch ($tt) {
            ...
            case KEYWORD_SWITCH:
                ...
                while (($tt = $this->t->get()) != OP_RIGHT_CURLY) {
                    ...
                    while (($tt = $this->t->peek()) != KEYWORD_CASE && $tt != KEYWORD_DEFAULT
                        && $tt != OP_RIGHT_CURLY) {
    ...
    private function Expression($x, $stop = false)
    {
        ...
        while (($tt = $this->t->get()) != TOKEN_END) {
            if ($tt == $stop && $x->bracketLevel == $bl && $x->curlyLevel == $cl && $x->parenLevel == $pl
                && $x->hookLevel == $hl
            ) {
            ...
            switch ($tt) {
                    ...
                    while (!$this->t->match(OP_RIGHT_CURLY)) {
                        do {
                            ...
                            if (($tv == 'get' || $tv == 'set') && $this->t->peek() == TOKEN_IDENTIFIER) {
                                ...
                                $this->t->mustMatch(OP_COLON);
                                $n->addNode(
                                    new JSNode($this->t, JS_PROPERTY_INIT, $id, $this->Expression($x, OP_COMMA))
                                );
                            }
                        } while ($this->t->match(OP_COMMA));
                ...
                case OP_RIGHT_PAREN:
                    ...
                    while (($tt = $this->reduce($operators, $operands)->type) != JS_GROUP && $tt != JS_CALL
                        && $tt != JS_NEW_WITH_ARGS
                    ) {
                        continue;
                    }
