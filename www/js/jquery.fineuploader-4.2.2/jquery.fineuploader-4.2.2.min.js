...
!function () {
    ...
    qq.log = function (a, b) {
        ...
        var d = new Date, e = "";
        c && (d.setTime(d.getTime() + 24 * c * 60 * 60 * 1e3), e = "; expires=" + d.toGMTString()), document.cookie = a + "=" + b + e + "; path=/"
...
}(), qq.version = "4.2.2", qq.supportedFeatures = function () {
    "use strict";
    function a() {
        var a, b = !0;
        try {
    ...
    var e = [], f = {}, g = {};
    qq.extend(this, {addFile: function (b, c, d, h) {
        h = h || qq.status.SUBMITTING;
        var i = e.push({name: c, originalName: c, uuid: b, size: d, status: h}) - 1;
    ...
    qq.basePublicApi = {log: function (a, b) {
        ...
        this.log("Resetting uploader..."), this._handler.reset(), this._storedIds = [], this._autoRetries = [], this._retryTimeouts = [], this._preventRetries = [], this._thumbnailUrls = [], qq.each(this._buttons, function (a, b) {
            b.reset()
        }), this._paramsStore.reset(), this._endpointStore.reset(), this._netUploadedOrQueued = 0, this._netUploaded = 0, this._uploadData.reset(), this._buttonIdsForFileIds = [], this._pasteHandler && this._pasteHandler.reset(), this._options.session.refreshOnReset && this._refreshSessionData()
    }, addFiles: function (a, b, c) {
        var d, e, f, g = [];
        if (a) {
            for (qq.isFileList(a) || (a = [].concat(a)), d = 0; d < a.length; d += 1)if (e = a[d], qq.isFileOrInput(e))if (qq.isInput(e) && qq.supportedFeatures.ajaxUploading)for (f = 0; f < e.files.length; f++)this._handleNewFile(e.files[f], g); else this._handleNewFile(e, g); else this.log(e + " is not a File or INPUT element!  Ignoring!", "warn");
        ...
        if (a) {
            ...
            qq.each(d, function (a, b) {
                var c;
                qq.isBlob(b) && !qq.isFileOrInput(b) ? c = {blob: b, name: f._options.blobs.defaultName} : qq.isObject(b) && b.blob && b.name ? c = b : f.log("addBlobs: entry at index " + a + " is not a Blob or a BlobData object", "error"), c && f._handleNewFile(c, e)
    ...
    }}, qq.basePrivateApi = {_uploadFile: function (a) {
        ...
        a.size >= 0 ? d = a.size : a.blob && (d = a.blob.size), c = this._uploadData.addFile(e, f, d), this._handler.add(c, a), this._netUploadedOrQueued++, b.push({id: c, file: a})
        ...
        }, getName: qq.bind(c.getName, c), getUuid: qq.bind(c.getUuid, c), getSize: qq.bind(c.getSize, c)};
    ...
    function j(a) {
        ...
        v.onSend(a), b = k(a, g), d(c) ? (c.onload = m(a), c.onerror = n(a)) : c.onreadystatechange = l(a), c.open(e, b, !0), v.cors.expected && v.cors.sendCredentials && !d(c) && (c.withCredentials = !0), o(a), r("Sending " + e + " request for " + a), h ? c.send(h) : s || !g ? c.send() : c.send(g && v.contentType.toLowerCase().indexOf("application/x-www-form-urlencoded") >= 0 ? qq.obj2url(g, "") : g && v.contentType.toLowerCase().indexOf("application/json") >= 0 ? JSON.stringify(g) : g)
    }
    ...
    function e() {
        ...
        h = new a["UploadHandler" + d](f, {onUploadComplete: c, onUuidChanged: f.onUuidChanged, getName: f.getName, getUuid: f.getUuid, getSize: f.getSize, log: g})
    }
    ...
    }}, qq.extend(f, a), g = f.log, qq.extend(this, {add: function () {
    ...
    "use strict";
    function d(b) {
        delete j[b], delete m[b], l && (clearTimeout(k[b]), delete k[b], t.stopReceivingMessages(b));
        var c = document.getElementById(a.getIframeName(b));
        c && (c.setAttribute("src", "java" + String.fromCharCode(115) + "cript:false;"), qq(c).remove())
    }

    function e(b, c) {
        var d = b.id, e = g(d), f = r(e);
        i[f] = c, j[e] = qq(b).attach("load", function () {
            m[e].input && (s("Received iframe load event for CORS upload request (iframe name " + d + ")"), k[d] = setTimeout(function () {
            ...
            h && i[h] ? (s("Handling response for iframe name " + d), clearTimeout(k[d]), delete k[d], a.detachLoadEvent(d), c = i[h], delete i[h], t.stopReceivingMessages(d), c(f)) : h || s("'" + b + "' does not contain a UUID - ignoring.")
        })
    }
    ...
    var h = qq.getUniqueId(), i = {}, j = {}, k = {}, l = b.isCors, m = b.fileState, n = b.inputName, o = c.onCancel, p = c.onUuidChanged, q = c.getName, r = c.getUuid, s = c.log, t = new qq.WindowReceiveMessage({log: s});
    qq.extend(a, {getIframeName: function (a) {
        ...
        var c;
        try {
            c = qq.parseJson(b), void 0 !== c.newUuid && p(a, c.newUuid)
        } catch (d) {
            s("Error when attempting to parse iframe upload response (" + d.message + ")", "error"), c = {}
        }
        return c
        ...
        l ? e(a, b) : j[a.id] = qq(a).attach("load", function () {
            if (s("Received response for " + a.id), a.parentNode) {
                try {
                    if (a.contentDocument && a.contentDocument.body && "false" == a.contentDocument.body.innerHTML)return
                } catch (d) {
                    s("Error when attempting to access iframe during handling of upload response (" + d.message + ")", "error"), c = {success: !1}
                }
                b(c)
            }
        })
    }, detachLoadEvent: function (a) {
        void 0 !== j[a] && (j[a](), delete j[a])
    }}), qq.extend(this, {add: function (a, b) {
        m[a] = {input: b}, b.setAttribute("name", n), b.parentNode && qq(b).remove()
    }, getInput: function (a) {
        return m[a].input
    }, isValid: function (a) {
    ...
    var e = this, f = b.fileState, g = b.chunking, h = c.onUpload, i = c.onCancel, j = (c.onUuidChanged, c.getName), k = c.getSize, l = c.log;
    qq.extend(a, {createXhr: function (a) {
        var b = qq.createXhrInstance();
        return f[a].xhr = b, b
    }, getTotalChunks: function (a) {
        if (g) {
            var b = k(a), c = g.partSize;
            return Math.ceil(b / c)
        }
    }, getChunkData: function (b, c) {
        var f = g.partSize, h = k(b), i = e.getFile(b), j = f * c, l = j + f >= h ? h : j + f, m = a.getTotalChunks(b);
        return{part: c, start: j, end: l, count: m, blob: d(i, j, l), size: l - j}
    }, getChunkDataForCallback: function (a) {
        return{partIndex: a.part, startByte: a.start + 1, endByte: a.end, totalParts: a.count}
    }}), qq.extend(this, {add: function (a, b) {
        if (qq.isFile(b))f[a] = {file: b}; else {
            if (!qq.isBlob(b.blob))throw new Error("Passed obj is not a File or BlobData (in qq.UploadHandlerXhr)");
            f[a] = {blobData: b}
        }
    }, getFile: function (a) {
        ...
        var b = f[a].xhr;
        b && (b.onreadystatechange = null, b.abort()), delete f[a]
    }, upload: function (a, b) {
        ...
        var b = f[a].xhr;
        return b ? (l(qq.format("Aborting XHR upload for {} '{}' due to pause instruction.", a, j(a))), f[a].paused = !0, b.abort(), !0) : void 0
    }})
    ...
    function t(a, b) {
        var c = i(a);
        c && !qq(c).hasClass(O.progressBar) && (c = qq(c).getByClass(O.progressBar)[0]), c && qq(c).css({width: b + "%"})
    }

    function u() {
        var a = N.placeholders.thumbnailNotAvailable, b = N.placeholders.waitingForThumbnail, c = {maxSize: M, scale: F};
    ...
    qq.extend(N, a), y = N.log, C = N.containerEl, E = void 0 !== N.imageGenerator, B = b(), u(), qq.extend(this, {render: function () {
        ...
        var b = a;
        if (b) {
            for (; null == b.getAttribute(G);)b = b.parentNode;
        ...
        var d, e = i(a);
        e && (d = Math.round(b / c * 100), b === c ? r(e) : s(e), t(a, d))
        ...
        var c = q(a), d = {maxSize: M, scale: !0, orient: !0};
        if (qq.supportedFeatures.imagePreviews) {
            if (P[a] = new qq.Promise, c)return v(c), N.imageGenerator.generate(b, c, d).then(function () {
                s(c), P[a].success()
            }, function () {
        ...
        var d = q(a), e = {maxSize: M, scale: F};
        if (d) {
            if (b)return c && v(d), N.imageGenerator.generate(b, d, e).then(function () {
                s(d)
            }, function () {
                w(a, d)
            });
    ...
    function c(a, b) {
        ...
        try {
            ...
            k("converting iframe's innerHTML to JSON"), k("innerHTML = " + e), e && e.match(/^<pre/i) && (e = d.body.firstChild.firstChild.nodeValue), c = l.parseJsonResponse(a, e)
        } catch (f) {
            k("Error when attempting to parse form upload response (" + f.message + ")", "error"), c = {success: !1}
        }
        return c
    }

    function d(b, c) {
        var d = a.paramsStore.getParams(b), e = a.demoMode ? "GET" : "POST", f = a.endpointStore.getEndpoint(b), g = h(b);
        return d[a.uuidName] = i(b), d[a.filenameParam] = g, l.initFormForUpload({method: e, endpoint: f, params: d, paramsInBody: a.paramsInBody, targetName: c.name})
    }

    var e = [], f = b.onUploadComplete, g = b.onUuidChanged, h = b.getName, i = b.getUuid, j = f, k = b.log, l = {};
    qq.extend(this, new qq.UploadHandlerFormApi(l, {fileState: e, isCors: a.cors.expected, inputName: a.inputName}, {onCancel: a.onCancel, onUuidChanged: g, getName: h, getUuid: i, log: k})), qq.extend(this, {upload: function (b) {
        ...
        a.onUpload(b, h(b)), f = d(b, m), f.appendChild(g), l.attachLoadEvent(m, function (d) {
            k("iframe loaded");
            var e = d ? d : c(b, m);
            l.detachLoadEvent(b), a.cors.expected || qq(m).remove(), (e.success || !a.onAutoRetry(b, i, e)) && (a.onComplete(b, i, e), j(b))
    ...
    function d(b, c, d) {
        var e = I(b), f = G(b);
        c[a.chunking.paramNames.partIndex] = d.part, c[a.chunking.paramNames.partByteOffset] = d.start, c[a.chunking.paramNames.chunkSize] = d.size, c[a.chunking.paramNames.totalParts] = d.count, c[a.totalFileSizeName] = e, O && (c[a.filenameParam] = f)
    }
    ...
    function f(b, c, d, e) {
        var f = new FormData, g = a.demoMode ? "GET" : "POST", h = a.endpointStore.getEndpoint(e), i = h, j = G(e), k = I(e);
        return b[a.uuidName] = H(e), b[a.filenameParam] = j, O && (b[a.totalFileSizeName] = k), a.paramsInBody || (O || (b[a.inputName] = j), i = qq.obj2url(b, h)), c.open(g, i, !0), a.cors.expected && a.cors.sendCredentials && (c.withCredentials = !0), O ? (a.paramsInBody && qq.obj2FormData(b, f), f.append(a.inputName, d), f) : d
    }

    function g(b, c) {
        var d = a.customHeaders, e = K[b].file || K[b].blobData.blob;
        c.setRequestHeader("X-Requested-With", "XMLHttpRequest"), c.setRequestHeader("Cache-Control", "no-cache"), O || (c.setRequestHeader("Content-Type", "application/octet-stream"), c.setRequestHeader("X-Mime-Type", e.type)), qq.each(d, function (a, b) {
    ...
    function h(b, c, d) {
        var e = G(b), f = I(b);
        K[b].attemptingResume = !1, a.onProgress(b, e, f, f), a.onComplete(b, e, c, d), K[b] && delete K[b].xhr, E(b)
    }

    function i(b) {
        var c, h, i = K[b].remainingChunkIdxs[0], k = P.getChunkData(b, i), l = P.createXhr(b), m = I(b), n = G(b);
        void 0 === K[b].loaded && (K[b].loaded = 0), N && K[b].file && t(b, k), l.onreadystatechange = s(b, l), l.upload.onprogress = function (c) {
            if (c.lengthComputable) {
                var d = c.loaded + K[b].loaded, e = j(b, i, c.total);
                a.onProgress(b, n, d, e)
            }
        }, a.onUploadChunk(b, n, P.getChunkDataForCallback(k)), h = a.paramsStore.getParams(b), d(b, h, k), K[b].attemptingResume && e(h), c = f(h, l, k.blob, b), g(b, l), J("Sending chunked upload request for item " + b + ": bytes " + (k.start + 1) + "-" + k.end + " of " + m), l.send(c)
    }

    function j(a, b, c) {
        var d = P.getChunkData(a, b), e = d.size, f = c - e, g = I(a), h = d.count, i = K[a].initialRequestOverhead, j = f - i;
        return K[a].lastRequestOverhead = f, 0 === b ? (K[a].lastChunkIdxProgress = 0, K[a].initialRequestOverhead = f, K[a].estTotalRequestsSize = g + h * f) : K[a].lastChunkIdxProgress !== b && (K[a].lastChunkIdxProgress = b, K[a].estTotalRequestsSize += j), K[a].estTotalRequestsSize
    }
    ...
    function l(b, c, d) {
        var e = K[b].remainingChunkIdxs.shift(), f = P.getChunkData(b, e);
        K[b].attemptingResume = !1, K[b].loaded += f.size + k(b), a.onUploadChunkSuccess(b, P.getChunkDataForCallback(f), c, d), K[b].remainingChunkIdxs.length > 0 ? i(b) : (N && u(b), h(b, c, d))
    }
    ...
    function o(a) {
        J("Server has ordered chunking effort to be restarted on next attempt for item ID " + a, "error"), N && (u(a), K[a].attemptingResume = !1), K[a].remainingChunkIdxs = [], delete K[a].loaded, delete K[a].estTotalRequestsSize, delete K[a].initialRequestOverhead
    }

    function p(a) {
        K[a].attemptingResume = !1, J("Server has declared that it cannot handle resume for item ID " + a + " - starting from the first chunk", "error"), o(a), Q.upload(a, !0)
    }
    ...
    function r(a, b) {
        ...
        d && !f && (J("xhr - server response received for " + a), J("responseText = " + b.responseText), c = n(a, b), m(b, c) ? (c.reset && o(a), e && c.reset ? p(a) : q(a, c, b)) : M ? l(a, c, b) : h(a, c, b))
    }
    ...
    function t(b, c) {
        var d = H(b), e = K[b].loaded, f = K[b].initialRequestOverhead, g = K[b].estTotalRequestsSize, h = w(b), i = d + L + c.part + L + e + L + f + L + g, j = a.resume.cookiesExpireIn;
        qq.setCookie(h, i, j)
    }
    ...
    function v(a) {
        ...
        if (h) {
            if (b = h.split(L), 5 === b.length)return c = b[0], d = parseInt(b[1], 10), e = parseInt(b[2], 10), f = parseInt(b[3], 10), g = parseInt(b[4], 10), {uuid: c, part: d, lastByteSent: e, initialRequestOverhead: f, estTotalRequestsSize: g};
    ...
    function w(b) {
        ...
        return c = "qqfilechunk" + L + encodeURIComponent(d) + L + e + L + f, void 0 !== D && (c += L + D), c
    }

    function x(a, b) {
        var c;
        for (c = P.getTotalChunks(a) - 1; c >= b; c -= 1)K[a].remainingChunkIdxs.unshift(c);
        i(a)
    }

    function y(a, b, c, d) {
        c = d.part, K[a].loaded = d.lastByteSent, K[a].estTotalRequestsSize = d.estTotalRequestsSize, K[a].initialRequestOverhead = d.initialRequestOverhead, K[a].attemptingResume = !0, J("Resuming " + b + " at partition index " + c), x(a, c)
    }

    function z(b, c, d) {
        var e, f = G(b), g = P.getChunkData(b, c.part);
        e = a.onResume(b, f, P.getChunkDataForCallback(g)), e instanceof qq.Promise ? (J("Waiting for onResume promise to be fulfilled for " + b), e.then(function () {
            y(b, f, d, c)
        }, function () {
    ...
    function A(a, b) {
        var c, d = 0;
        K[a].remainingChunkIdxs && 0 !== K[a].remainingChunkIdxs.length ? i(a) : (K[a].remainingChunkIdxs = [], N && !b && K[a].file ? (c = v(a), c ? z(a, c, d) : x(a, d)) : x(a, d))
    }

    function B(b) {
        var c, d, e, h = K[b].file || K[b].blobData.blob, i = G(b);
        K[b].loaded = 0, c = P.createXhr(b), c.upload.onprogress = function (c) {
            c.lengthComputable && (K[b].loaded = c.loaded, a.onProgress(b, i, c.loaded, c.total))
        }, c.onreadystatechange = s(b, c), d = a.paramsStore.getParams(b), e = f(d, c, h, b), g(b, c), J("Sending upload request for " + b), c.send(e)
    }

    function C(b, c) {
        var d = G(b);
        Q.isValid(b) && (a.onUpload(b, d), M ? A(b, c) : B(b))
    }
    ...
    D = c(), qq.extend(this, new qq.UploadHandlerXhrApi(P, {fileState: K, chunking: M ? a.chunking : null}, {onUpload: C, onCancel: a.onCancel, onUuidChanged: F, getName: G, getSize: I, getUuid: H, log: J})), qq.override(this, function (b) {
        return{add: function (a) {
            var c;
            return b.add.apply(this, arguments), N && (c = v(a), c && F(a, c.uuid)), a
        }, getResumableFilesData: function () {
            var b = [], c = [];
            return M && N ? (b = qq.getCookieNames(void 0 === D ? new RegExp("^qqfilechunk\\" + L + ".+\\" + L + "\\d+\\" + L + a.chunking.partSize + "=") : new RegExp("^qqfilechunk\\" + L + ".+\\" + L + "\\d+\\" + L + a.chunking.partSize + "\\" + L + D + "=")), qq.each(b, function (a, b) {
                var d = b.split(L), e = qq.getCookie(b).split(L);
    ...
    function e(a) {
        var c = new qq.UploadDropZone({HIDE_ZONES_EVENT_NAME: j, element: a, onEnter: function (b) {
    ...
    qq.extend(d, a), c = new qq.AjaxRequester({validMethods: ["GET"], method: "GET", endpointStore: {getEndpoint: function () {
        return d.endpoint
        ...
        a.qqtimestamp = (new Date).getTime(), d.log("Session query request."), c.initTransport("sessionRefresh").withParams(a).send()
    }})
